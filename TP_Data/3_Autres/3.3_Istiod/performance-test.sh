...
main(){
  ...
  SLEEP_POD=$(kubectl -n formation get pod -l app=sleep -o jsonpath={.items..metadata.name} -n formation | cut -d ' ' -f 1)
  ...
  while [[ "$(curl --max-time 0,5 -s -o /dev/null -H "Host: $SERVICE_NAME.formation.io" -w ''%{http_code}'' $GATEWAY/items)" != "200" ]]; do 
    # curl --max-time .5 -s -o /dev/null -H "Host: $SERVICE_NAME.formation.io" $GATEWAY/items
    sleep .2
  done
...
spec:
  ...
  - route:
    - destination:
        host: $SERVICE_NAME.formation.svc.cluster.local
        port:
          number: 80
---
EOF
...
Poor Man's Performance Test creates Services, Gateways and VirtualServices and measures Latency and Push Count needed to distribute the updates to the data plane.
       ...
       --namespace    Namespace in which to create the resources. Default 'formation'
